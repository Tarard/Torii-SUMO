...
The current OSM tools cover bounded bbox/source-OSM network construction, tiled Overpass requests, retry, OSM XML deduplication, road-class filtering, TLS audit candidate extraction, TLS clustering, Google Maps URL emission with temporal-scope fields, passenger connectivity checks, Netedit launch evidence, and named-road routeability probe generation. The high-level cleanup workflow coordinates these tools and blocks or demotes claims when area confirmation, map/TLS review, connectivity, or Netedit inspection evidence is missing. They do not by themselves prove signal timing, phasing, or controller readiness.
