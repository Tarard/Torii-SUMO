...
def write_teacher_connection_plan(
    *,
    ...
    if generate_structural_connections and structural_junction_ids and patched_edge_endpoints:
        ...
        incoming = [edge_id for edge_id, (_, target) in patched_edge_endpoints.items() if target in structural_scope]
        outgoing = [edge_id for edge_id, (source, _) in patched_edge_endpoints.items() if source in structural_scope]
    ...
    *,
    ...
    for teacher_edge_id in boundary_edge_ids:
        ...
        if (
            ...
            and (
                teacher_edge_id in pair_map or (requested_source == "base_plan_edge_map" and chosen == teacher_edge_id)
            )
        ):
...
def restore_scoped_pedestrian_internal_semantics_after_normalize(
    *,
    ...
    source_pedestrian_edges = {
        ...
        if edge.attrib.get("id", "").startswith(internal_prefix) and edge.attrib.get("function") in pedestrian_functions
    }
    ...
    def mapped_pedestrian_edge(source_edge: ET.Element) -> ET.Element:
        ...
        if clone.attrib.get("crossingEdges"):
            clone.set(
                "crossingEdges",
                " ".join(
                    edge_map.get(value, value) for value in clone.attrib.get("crossingEdges", "").split() if value
                ),
            )
        return clone
...
def build_teacher_guided_junction_variant(
    *,
    ...
    for compound_junction_id in compound_junction_ids:
        ...
        missing_boundary_edge_ids = sorted(raw_missing_boundary_edge_ids - set(internalized_boundary_edge_aliases))
    ...
    *,
    ...
    for index, candidate in enumerate(candidates):
        ...
        if candidate.get("candidate_status") == "needs_expanded_rebuild_scope" and junction_id:
            ...
            # The node file carries the <join>; netconvert must own endpoint
            # rewiring after it creates the joined node.
            rewrite_joined_endpoints = False
            ...
            if (
                ...
                variant_replay_target_internal_subgraph = (
                    candidate_replay_target_internal_subgraph and not use_full_network_join_patch_replay
                )
                ...
                try:
                    variant_report = variant_builder(
                        ...
                        structural_osm_boundary_authority=use_full_network_join_patch_replay,
...
def _normalized_junction_request_model(
    ...
    for lane_id in internal_lanes:
        ...
        if len(keys) > 1:
            ...
            failures.append(
                {
                    "reason": "contraction_neighbor_request_lane_unmapped",
                    "junction_id": junction_id,
                    "lane_id": lane_id,
                }
            )
