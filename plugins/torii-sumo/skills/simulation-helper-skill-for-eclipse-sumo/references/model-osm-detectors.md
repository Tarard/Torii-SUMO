...
When a reference `.net.xml` is supplied and the user has not supplied a bbox or source OSM extract, derive the OSM download bbox from the reference network's actual non-internal junction and lane geometry with a small meter-scale padding. Do not trust `.net.xml` `origBoundary` as the construction scope because it can remain stale after a network has been clipped from a larger source. Report `reference_bbox_status`, `reference_bbox`, `reference_orig_boundary`, and the bbox source in the workflow result.
...
Before building a correction variant, run `sumo_network_overlapping_junction_audit` when the visual issue is stacked red junction dots. It is read-only and separates suspicious co-planar top-level junctions from valid SUMO internal crossing and walkingarea layers. Feed overlap groups into `sumo_network_junction_aggregation_variant` only after reference or human review confirms the core nodes to join.
