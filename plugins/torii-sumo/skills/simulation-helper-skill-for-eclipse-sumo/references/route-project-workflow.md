...
| SUMO, TraCI, route, detector, TLS, output, seed, or completion failure | `preflight-sumo-environment.md`, then `develop-and-verify-code.md` when code/config changes are needed | root-cause hypothesis, next probe, fix/rerun/demotion |
...
- If the deviation is runtime behavior, collect preflight/run/output evidence first; use `develop-and-verify-code.md` before changing controller logic.
