...
| Need direct NetEdit mouse/keyboard control with visual feedback | `sumo_netedit_session` | Use the grouped `open -> observe -> act -> observe -> finalize/abort` session. Keep one active session and perform one whitelisted action per call against the caller's exact last recorded screenshot SHA followed by a fresh live capture. Bounded global animation may change for coordinate actions, but click/drag targets must remain exact; every semantic shortcut/save requires an exact live viewport match. F7 additionally requires a frozen selection containing exactly the declared source junction ids and must be the first edit action. Treat `persisted_scene` as on-disk XML only; screenshots may include unsaved live state. Finalized candidates never promote automatically. |
...
| Need the detector-constrained SUMO replay after signal history passes | `sumo_hamburg_sandtorkai_named_replay` | Supply the execution-ready signal-observation manifest to select TraCI dynamic TLS replay; without it the all-red construction path remains diagnostic and must fail the teleport/collision gate |
| Need to resume or inspect the Codex execution plan for the named corridor | `sumo_hamburg_sandtorkai_execution_plan` | Hash-check W0-W5 manifests, show dependency readiness, invalidate downstream stages after an upstream change, and stop automatically on missing official evidence |
