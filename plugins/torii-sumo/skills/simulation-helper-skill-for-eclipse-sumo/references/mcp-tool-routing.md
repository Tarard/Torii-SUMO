...
| Need OSM cleanup from place name, bbox, or extract with mandatory gates | `sumo_osm_cleanup_workflow` | Enforce area confirmation, OSM build, mandatory Google Maps TLS review gate, physical TLS clustering, TLS aggregation review variants, passenger connectivity, connected-core extraction, scale-derived routeability audit parameters, optional routeability probes, reference visual-detail join audit, SUMO-GUI/Netedit launch, and claim boundary |
