...
> [!IMPORTANT]
> Torii 不是单纯的提示词或说明文档。它把 SUMO expert skill 和本地 stdio MCP 工具打包在一起，让 Codex/Claude 这类 coding agent 能够执行有限、可验证的 SUMO 检查。当前版本已经包含环境检查、配置预检、冒烟运行、证据包、OSM 清洗硬门槛流程、OSM-to-SUMO 构网、TLS 候选审计、连接性检查、Netedit 打开证据和路线可达性检查；静默完整地名解析、整城级自动认证和控制器自动生成还不是已经完成的 MCP 工具。
...
- 可以做道路筛选、OSM 去重、TLS 候选、Google Maps/用户目标时间对齐的审查材料、连接性检查、路线可达性、warning 观察和 Netedit 打开证据；不能声称自动认证整个城市路网。
- 默认把 Google Maps 当作当前路网现实基线；如果用户要求历史路网，则以用户声明的历史目标为准，并需要时间对齐的证据。
