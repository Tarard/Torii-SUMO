...
|---|---:|---:|---:|---:|---:|---:|---|
| 矩阵诊断，关闭 TLS，完整 150 分钟 | 60/60 | 5,928 | 5,928 | 0 | 0.00 | 0 | 逆需求诊断精确匹配 |
| 矩阵诊断，关闭 TLS，正式两小时 | 48/48 | 4,809 | 4,809 | 0 | 0.00 | 0 | 正式窗口诊断精确匹配 |
| 使用现有官方历史信号严格回放 7,200 秒 | 78/78 | 3,936 | 2,501 | −1,435 | 26.47 | 84 | 尚未通过数字孪生验收 |

route 复原明确是一个**非唯一解**。Torii 先枚举 58 条可能经过走廊或支路的路线，再让每条路线各运行一辆探针车；按照汉堡官方站点 composition 规则聚合物理 SUMO E1 响应，由此得到传感器响应矩阵 `H`。随后对每个 15 分钟片求解：

```text
最小化    ||x_t - x_t_prior||_1
约束条件  H x_t = y_t
          x_t >= 0 且为整数
```
