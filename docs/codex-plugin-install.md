...
- `sumo_osm_cleanup_workflow`: run the hard-gate OSM cleanup workflow, including area confirmation when needed, network construction, TLS map audit, passenger connectivity summary, connected-core extraction when needed, routeability probes when supplied, and Netedit launch evidence.
