...
No source files from `XRDS76354/SUMO-MCP-Server` are copied in the initial MVP plan. If a later task copies source code from that repository, preserve its MIT license and copyright notice.

Some evidence-inspection design patterns are adapted from the local `SUMO-Virtual-Experiment-Sidecar` project owned by the same user.
